...
class Crate extends LSprite implements PhysicsEntity implements Groundable {
	...
	public static var CRATE_FILTER:InteractionFilter = new InteractionFilter();
	...
	public function new() {
		...
		CRATE_FILTER.collisionGroup = (1 << 7);
		CRATE_FILTER.collisionMask = ~Spike.SPIKE_FILTER.collisionGroup;

		for (shape in physics.body.shapes) {
			shape.filter = CRATE_FILTER;
		}
